...
and wires together the SQL generation and schema detection services.
"""
...
def main() -> None:
    ...
    # ── Page header ───────────────────────────────────────────────────────────
    st.title("🗄️ Eng2SQL — English to SQL Generator")
    st.markdown(
        "Type a question in plain English and get a valid **MySQL** query instantly. "
    ...
    with col_output:
        ...
        output_component.render(st.session_state["generated_sql"])
    ...
    if question:
        ...
        try:
            ...
            if schema is None:
                st.warning(
                    "⚠️ Please connect to a database and select it in the sidebar first."
                )
                return
            ...
            # Step 3: Generate SQL
            progress.update("Step 3/4 — Generating SQL with OpenAI…")
    ...
    if (
        ...
        and st.session_state.get("db_engine")
    ):
        if st.button("▶ Execute SQL", type="secondary"):
            try:
                engine = st.session_state["db_engine"]
                result_df = connector.execute_query(engine, st.session_state["generated_sql"])
